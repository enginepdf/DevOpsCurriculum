...
## Quest
* 지금까지 만든 세 개의 자바스크립트 코드베이스(컨테이너 기반 API, 람다 기반 API, 정적 웹사이트)를 Lerna를 이용해 하나의 리포로 합쳐 보세요.(O) https://github.com/enginepdf/Quest
...
* eslint의 기본 설정을 이용해 이 코드들을 린팅해 보세요. 모든 패키지를 명령 하나로 한 번에 린팅하려면 어떻게 해야 할까요?(O)
        ...
        npx lerna run lint (모든 패키지를 린팅)
            컨테이너 기반 API, 람다 기반 API, 정적 웹사이트 관련 패키지 린팅 작동 확인